...
* 罗尔定理 
  * **费马引理**：在$x_0$的邻域内，始终$f(x)\le f(x_0)(或f(x)\ge f(x_0))$，若$f(x)$可导，那么$f\acute(x_0)=0$
    * **驻点**：*导数为0的点*
  * **罗尔定理**
    * $[a,b]连续$
    * $(a,b)可导$
    * $f(a)=f(b)$，那么必然存在$f\acute(\xi)=0$
* **拉格朗日中值定理**
  * $[a,b]$连续
  * $(a,b)可导$
  * 那么必然存在$f(a)-f(b)=f\acute(\xi)(a-b)$
  * **推论**
    * 如果一个函数在区间上可导且导数为0，**那么该函数为常数函数**
* **柯西中值定理**
  * $[a,b]$连续
  * $(a,b)可导$
  * $F\acute(x)\ne 0$，那么存在$\frac{f(a)-f(b)}{F(a)-F(b)}=\frac{f\acute(\xi)}{F\acute(\xi)}$
...
* **单调性的判定法**
  * $[a,b]连续，(a,b)可导$
  * $若f\acute(x)\le0，且在有限多个点上导数为0，那么该函数单调递减$
  * $若f\acute(x)\ge0，且在有限多个点上导数为0，那么该函数单调递增$
* **凹凸性与拐点**
  * **凹凸性的定义**
  ...
  * **拐点**：凹凸性改变的点
    * **拐点的判定过程**
      1. 求$f^{(2)}(x)$
      2. 令$f^{(2)}(x)=0$，解出这方程在区间I内的实根，并求出在区间I内的二阶导数不存在的点
      3. 判断左右二阶导数是否异号，最终筛选出拐点
...
* **函数极值及其求法**
  ...
  * **极值**：函数某一个点周围的值都小于(大于)该点的值，那么这就是极值点
    ...
    * **局部性**
    * **必要条件**：如果函数在极值点可导，那么导数为0
    * **充分条件**
      1. **第一充分**：如果一个点左右两侧函数单调性改变，该点为极值点
      2. **第二充分**：一阶导数为0+二阶导数符号确定极大极小值
  ...
  * **求极值点步骤**
    ...
    1. **方法一**
       ...
       * 根据一阶导数等于0以及不可导点，锁定备选集

       * 利用充分条件筛选极值点(判断每个点左右两侧的一阶导数正负)

    2. **方法二**

       * 求一阶导数
       * 求驻点以及不可导点
       * 判断驻点的二阶导数，以及不可导点的两侧单调性变化
